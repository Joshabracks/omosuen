 ...
 */
async function createBranchingTree(
  depth: number,
  branchFactor: number,
): Promise<nexus> {
  let nodeCounter = 0;
  ...
  console.log(
    `${colors.gray}📊 Created tree: ${nodeCounter} nodes (depth=${depth}, branch=${branchFactor})${colors.reset}`,
  );
  return root;
}
...
async function testAddComponent(root: nexus): Promise<void> {
  ...
  assert(
    found !== null && found.name === 'TestNode-Add',
...
async function testAddComponents(root: nexus): Promise<void> {
  ...
  assert(
    batch1 !== null && batch2 !== null && batch3 !== null,
...
function testGetComponentsByType(root: nexus): void {
  ...
  assert(
    Array.isArray(immediateChildren) && immediateChildren.length >= 5,
  ...
  assert(
    Array.isArray(allNexuses) && allNexuses.length > 100,
  ...
  assert(
    Array.isArray(noSprites) && noSprites.length === 0,
...
function testGetComponentByName(root: nexus): void {
  ...
  assert(
    immediateChild !== null && immediateChild.name === '0',
  ...
  assert(
    deepNode !== null && deepNode.name === '0-1-2-3',
  ...
  assert(notFound === null, "Non-recursive doesn't find deep nodes");

  // Test 4: Non-existent name returns null
  const noMatch = $.getComponentByName(root, 'NonExistentName', true);
  assert(noMatch === null, 'Non-existent name returns null');
}
...
async function testGetComponentsByName(root: nexus): Promise<void> {
  ...
  assert(
    unique.length === 1 && unique[0].name === '0-2',
  ...
  assert(none.length === 0, 'Non-existent name returns empty array');
}
...
function testGetComponentById(root: nexus): void {
  ...
  assert(
    foundChild !== null && foundChild.id === childId,
...
function testGetComponentsById(root: nexus): void {
  ...
  assert(noMatches.length === 0, 'Non-existent ID returns empty array');
  ...
  assert(
    nonRecursive.length > 0 || recursive.length > 0,
...
async function testGetComponentByTypeAndName(root: nexus): Promise<void> {
  ...
  assert(
    found !== null && found.name === '0-1' && found.type === 'nexus',
  ...
  assert(wrongType === null, 'Wrong type returns null');

  // Test 3: Wrong name returns null
  const wrongName = $.getComponentByTypeAndName(
    root,
    'nexus',
    'NoSuchName',
    true,
  );
  assert(wrongName === null, 'Wrong name returns null');

  // Test 4: Non-recursive limitation
  const deep = $.getComponentByTypeAndName(root, 'nexus', '0-1-2-3', false);
  assert(deep === null, "Non-recursive doesn't find deep nodes");
}
...
async function testGetComponentsByTypeAndName(root: nexus): Promise<void> {
  ...
  const dupes = $.getComponentsByTypeAndName(
    root,
    'nexus',
    'TypeAndName',
    false,
  );
  assert(dupes.length === 2, `Find multiple matches (got ${dupes.length})`);

  // Test 3: Wrong type returns empty array
  const wrongType = $.getComponentsByTypeAndName(root, 'sprite', 'Root', true);
  assert(wrongType.length === 0, 'Wrong type returns empty array');
}
...
function testDispose(root: nexus): void {
  ...
  const childDisposed = childrenBefore.every(
    (c: ComponentData) => c._disposed === true,
  );
...
async function runTests(): Promise<void> {
  ...
  console.log(
    `${colors.blue}║   Omosuen Nexus Component Unit Tests   ║${colors.reset}`,
  );
  console.log(
    `${colors.blue}╚════════════════════════════════════════╝${colors.reset}\n`,
  );
  ...
  try {
    ...
    console.error(
      `${colors.red}✗ Failed to create test tree:${colors.reset}`,
      error,
    );
    process.exit(1);
  }
