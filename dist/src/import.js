...
export async function importAsepriteSources(entries, config) {
    ...
    const buffers = await Promise.all(entries.map(async (e) => {
        const res = await fetch(e.filePath);
        if (!res.ok) {
            console.error(`[aseprite-loader] failed to fetch source '${e.filePath}': ${res.status} ${res.statusText}`);
            return null;
        }
    ...
    for (let ei = 0; ei < entries.length; ei++) {
        ...
        const buildNames = entry.flatten
            ? [entry.id]
