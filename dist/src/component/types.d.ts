...
export type COMPONENT_TYPE = 'nexus' | 'ui-overlay' | 'data-layer' | 'flag-manager' | 'messenger' | 'viewport' | 'texture-map' | 'atlas-manager' | 'sprite' | 'transform' | 'animation-controller' | 'animation-map' | 'cell-map' | 'camera' | 'input-controller' | 'collider' | 'event-collider' | 'timer' | 'speed-dial' | 'light' | 'audio-track' | 'audio-effect' | 'audio-player';
...
export declare function newComponent(type: COMPONENT_TYPE, options: ComponentOptions, parent?: NexusT | null): Promise<ComponentData | null>;
