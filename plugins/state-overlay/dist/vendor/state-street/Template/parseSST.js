...
(function (ELEMENT_TYPE) {
    ELEMENT_TYPE[ELEMENT_TYPE["OPEN"] = 0] = "OPEN";
    ELEMENT_TYPE[ELEMENT_TYPE["CLOSING"] = 1] = "CLOSING";
    ELEMENT_TYPE[ELEMENT_TYPE["TEXT"] = 2] = "TEXT";
    ELEMENT_TYPE[ELEMENT_TYPE["SELF_CLOSING"] = 3] = "SELF_CLOSING";
...
function getElements(data, components, pos, content, stash) {
    const len = data.length;
    while (pos < len) {
        // Skip whitespace between tokens (replaces the old per-iteration .trim()).
        while (pos < len && isWhitespace(data.charCodeAt(pos)))
            pos++;
        if (pos >= len)
            break;
        let elementType;
        ...
        if (elementType === ELEMENT_TYPE.OPEN) {
            ...
            const rawIdx = attributes.findIndex((a) => a.name === 'data-ssrawid');
            if (rawIdx !== -1) {
                const entry = stash[Number(attributes[rawIdx].value)];
                attributes.splice(rawIdx, 1);
                const node = { type: tagName, attributes, events, content: [entry.inner] };
                if (entry.kind === 'raw') {
                    node.raw = true;
                    node.rawFormatter = entry.formatter;
                }
                content.push(node);
                pos = child.pos;
                continue;
            ...
            if (/(^|\s):preserve\b/i.test(elementString))
                node.preserve = true;
            content.push(node);
            pos = child.pos;
            continue;
...
function extractRaw(data, stash) {
    let out = data;
    // 1. `:raw` (optionally `:raw=formatter`) on any element.
    out = out.replace(/<([a-zA-Z][\w-]*)([^>]*?)\s:raw(?:=(\w+))?([^>]*?)>([\s\S]*?)<\/\1\s*>/g, (_m, tag, pre, fmt, post, inner) => {
    ...
    out = out.replace(/<(script|style|code)(?![^>]*data-ssrawid)((?:\s[^>]*)?)>([\s\S]*?)<\/\1\s*>/gi, (_m, tag, attrs, inner) => {
        const id = stash.length;
    ...
    out = out.replace(/<(textarea|title)(?![^>]*data-ssrawid)((?:\s[^>]*)?)>([\s\S]*?)<\/\1\s*>/gi, (_m, tag, attrs, inner) => {
        const id = stash.length;
...
function parseSST(data, components = {}) {
    ...
    processed = processed.replace(/<!--\s+([\w]+)((?:\s+[\w]+="[^"]*")*)\s*\/\s*-->/g, (_match, name, attrs) => components[name] ? `<${name}${attrs} />` : '');
