...
const scrolledSSIDs = new Set();
let scrollListenerInstalled = false;
function ensureScrollListener() {
    if (scrollListenerInstalled || typeof document === "undefined")
        return;
    ...
    document.addEventListener("scroll", (e) => {
        ...
        if (id)
            scrolledSSIDs.add(id);
    }, { capture: true, passive: true });
}
function captureScroll(componentSSID, state) {
    if (scrolledSSIDs.size === 0)
        return null;
    let stash = null;
    scrolledSSIDs.forEach((id) => {
        if (!id.startsWith(componentSSID))
            return;
...
function restoreScroll(stash, state) {
...
function captureFocus(componentSSID) {
    const active = document.activeElement;
    if (!(active instanceof Element))
        return null;
    const id = active.getAttribute(SSID);
    if (!id || !id.startsWith(componentSSID))
        return null;
...
function restoreFocus(snap, state) {
...
function rebuildComponentRange(rec, ssid, state) {
    ...
    for (let i = 0; i < parsedBody.length; i++) {
        ...
        const subElement = constructElement(parsedBody[i], childSSID, state, ns);
        if (subElement) {
...
function updateDOM(state) {
    ...
    for (let i = 0; i < ssids.length; i++) {
        ...
        if (!rec.startMarker.isConnected)
            continue;
        if (rec.tick === state.tick)
            continue;
        ...
        if (changed) {
            if (stash)
                restoreScroll(stash, state);
            if (focusSnap)
                restoreFocus(focusSnap, state);
        }
    }
    const { textMap, dirtyKeys } = state;
    ...
    for (const ssid in state.nodeMap) {
        if (!state.nodeMap[ssid].isConnected)
            delete state.nodeMap[ssid];
    }
    ...
    for (const ssid in state.componentMap) {
        ...
        if (!r.startMarker.isConnected && r.tick !== state.tick)
            delete state.componentMap[ssid];
    }
}
