...
import { setProfilingEnabled, getLastFrameProfile, getFrameHistory, } from 'omosuen';
const PHASE_ORDER = [
    'init',
    'update',
    'dispose',
 ...
 */
function buildRankedTable(columns, rowCount) {
...
function builder(options) {
    ...
    const phaseCells = PHASE_ORDER.map((phase) => {
    ...
    const { tableEl: instanceTableEl, tbodyEl: instanceTbodyEl, rowPool: instanceRowPool, } = buildRankedTable([
...
function setVisible(p, visible) {
    p.visible = visible;
    p.container.style.display = visible ? 'block' : 'none';
}
function drawGraph(canvas, frameTimes) {
 ...
 */
function averageByInstance(frames) {
    const totals = new Map();
    for (const frame of frames) {
...
function render(p) {
    ...
    if (last.timestamp - p._lastTableRefresh >= TABLE_REFRESH_MS) {
        const windowFrames = getFrameHistory().filter((f) => f.timestamp >= p._lastTableRefresh - TABLE_REFRESH_MS);
 ...
 */
export const perfMonitorDefinition = {
    type: 'perf-monitor',
    builder: builder,
    methods,
