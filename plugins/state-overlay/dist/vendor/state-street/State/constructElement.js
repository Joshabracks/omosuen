...
export function setAttr(element, name, value) {
    ...
    if (ci > 0) {
        ...
        if (prefix === "xml") {
            element.setAttributeNS(XML_NS, name, value);
            return;
        }
    }
...
export function childNamespaceOf(parent) {
...
export function getValue(obj, values) {
    if (values.length === 0)
        return obj;
...
export function unescapeQuotes(str) {
    return str.replace(/\\(["'`\\])/g, "$1");
}
function coerceArg(raw, state) {
    const value = (raw ?? "").trim();
    const varMatch = value.match(cleanerRegex);
    if (varMatch)
        return getValue(state.data, varMatch[1].split("."));
    const q = value[0];
...
export function runComponent(data, state) {
    ...
    const props = {};
    for (const k in cp)
        props[k] = cp[k] === undefined ? true : coerceArg(cp[k], trackedState);
...
export function parseComponentBody(componentBody, state) {
...
function constructElement(data, parentSSID, state, ns) {
    ...
    if (tag === "_component") {
        ...
        for (let i = 0; i < parsedBody.length; i++) {
            ...
            const subElement = constructElement(parsedBody[i], ssid, state, ns);
            if (subElement)
    ...
    const noCache = isImg && attributes.some((a) => a.name === "nocache");
    const desired = new Map();
    attributes.forEach((attribute) => {
        if (isImg && attribute.name === "nocache")
            return;
    ...
    if (isImg && !desired.has("decoding"))
        desired.set("decoding", "async");
    if (reuse) {
        ...
        if (!preserve) {
            for (const attr of Array.from(element.attributes)) {
                if (attr.name !== SSID && attr.name !== STID && !desired.has(attr.name))
                    element.removeAttribute(attr.name);
            }
        }
        desired.forEach((v, n) => { if (element.getAttribute(n) !== v)
            setAttr(element, n, v); });
    }
    else {
    ...
    const prev = element.__sstEvents;
    if (prev)
        for (const p of prev)
            element.removeEventListener(p.type, p.fn);
    const bound = [];
    ...
    events.forEach((event) => {
        ...
        const fn = (e) => state.methods[event.function]({ ...eventProps, event: e, state });
        element.addEventListener(event.type, fn);
        bound.push({ type: event.type, fn });
    });
    element.__sstEvents = bound;
    ...
    element.moveTo = (target) => state.moveElement(currentSSID, element, target);
    element.resetLocation = () => state.resetElementLocation(currentSSID, element);
    ...
    if (REUSABLE_TAGS.has(tag))
        state.nodeMap[currentSSID] = element;
    ...
    if (data?.preserve && state.preserveSet)
        state.preserveSet.add(currentSSID);
    ...
    if (data?.raw) {
        ...
        if (data.rawFormatter) {
            ...
            if (fn)
                element.innerHTML = `${fn({ text: rawText, state })}`;
            else {
                console.error(`invalid :raw formatter: ${data.rawFormatter}`);
