 ...
 */
async function initFromSources(a, parent, atlasManager) {
    const seen = new Set();
    const deduped = (a.sources ?? []).filter((s) => {
        if (seen.has(s.filePath)) {
            console.warn(`[aseprite-loader] '${a.name}' skipping duplicate source '${s.filePath}'`);
            return false;
        }
        seen.add(s.filePath);
        return true;
    });
    const entries = await Promise.all(deduped.map(async (s) => {
        const response = await fetch(s.filePath);
        if (!response.ok) {
            console.error(`[aseprite-loader] '${a.name}' failed to fetch source '${s.filePath}': ${response.status} ${response.statusText}`);
            return null;
        }
    ...
    if (resolved.length === 0) {
        console.warn(`[aseprite-loader] '${a.name}' has no loadable sources; nothing imported`);
        return;
    }
