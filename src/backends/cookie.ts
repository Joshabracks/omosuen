...
import type { KVBackend, CookieConfig } from '../types.js';
...
export function cookieBackend(cfg: CookieConfig): KVBackend {
  // value === null deletes the cookie. Returns false if not written.
  const write = (fullKey: string, value: string | null): boolean => {
    ...
    if (cookie.length > MAX_COOKIE_BYTES) {
      console.warn(
        `[browser-local-storage] cookie '${fullKey}' exceeds ~4KB (${cookie.length}b) — not written`,
      );
      return false;
    }
  ...
  return {
    ...
    async clear(prefix) {
      for (const k of Object.keys(parseCookies())) {
        if (k.startsWith(prefix)) write(k, null);
      }
    },
  };
}
