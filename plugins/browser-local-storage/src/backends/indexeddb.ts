...
export function idbBackend(dbName: string, storeName: string): KVBackend {
  ...
  return {
    ...
    async remove(fullKey) {
      const s = await store('readwrite');
      if (s) await req(s.delete(fullKey));
    },
    async has(fullKey) {
      const s = await store('readonly');
      if (!s) return false;
    ...
    async keys(prefix) {
      const s = await store('readonly');
      if (!s) return [];
      const all = (await req(s.getAllKeys())) as IDBValidKey[];
      return all
        .map(String)
    ...
    async clear(prefix) {
      ...
      const all = (await req(s.getAllKeys())) as IDBValidKey[];
      for (const k of all) {
        if (String(k).startsWith(prefix)) await req(s.delete(k));
      }
    },
  };
}
